...
my $stmt = 'SELECT account_id FROM public.account';
...
sub set_info_common {
	...
	my $count_stmt = 'SELECT COUNT(*) FROM public.info_common WHERE account_id=\'' . $info_common{'account_id'} . '\'';
	...
	our($stmt);
	if ( $count eq 0 ) {
		$stmt = 'INSERT INTO public.info_common ( account_id, 
		                                          clan_id,
		                                          client_language,
		                                          global_rating,
		                                          created_at,
		...
		$stmt = 'UPDATE public.info_common
...
sub set_info_statistics_max {
	...
	my $count_stmt = 'SELECT COUNT(*) FROM public.info_statistics_max WHERE account_id=\'' . $info_statistics_max{'account_id'} . '\'';
	my $count = $dbh->selectrow_array( $count_stmt );
	if ($dbh->err()){
		die $count_stmt . ' failed : ' . $dbh->errstr();
	}
	...
	our($stmt);
	if ( $count eq 0 ) {
		$stmt = 'INSERT INTO public.info_statistics_max ( account_id, 
														  max_xp_tank_id,
														  max_xp,
														  max_damage_vehicle,
														  max_damage_tank_id,
		...
		$stmt = 'UPDATE public.info_statistics_max
